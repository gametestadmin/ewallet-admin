<form class="form-horizontal col-xs-12">
    ...
    <ul class="list-inline header-list text-center">
      ...
      <li class="{% if realParent == 1 or realParent == 3 %}col-xs-8{%else%}col-xs-10{%endif%} list-group-item">{{translate['head_list_ip']}}</li>
      {% if realParent == 1 or realParent == 3 %}
        <li class="col-xs-2 list-group-item">{{translate['head_list_delete']}}</li>
      {% endif %}
    </ul>
            ...
            <ul class="list-inline {{class}} text-center">
                ...
                {% if realParent == 1 or realParent == 3 %}
                    ...
                    <!--<span class="ip-edit fa fa-edit text-primary" data-id="{{userWhitelistIpData.id}}" data-toggle="modal" data-target="#form-edit-user-ip"></span>-->
                        <a href="{{url(module~'/whitelist/delete/'~agent.id~'?delete='~userWhitelistIpData.id)}}" class="delete">
                            <i class="fa fa-ban text-danger" data-toggle="tooltip" data-placement="right" title="{{translate['text_delete']}}"></i>
                        </a>
                    </li>
                {% endif %}
            </ul>
